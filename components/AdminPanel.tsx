...
const AdminPanel: React.FC<AdminPanelProps> = ({ 
  ...
  const [history, setHistory] = useState<{projects: Project[], content: SiteContent}[]>([]);
  ...
  const handleSave = () => {
    ...
    setHistory([]); // Optional: Clear history on save
  ...
  const handleProjectChange = (field: keyof Project, value: any) => {
    ...
    // Note: We don't save history on every keystroke here to avoid spamming.
    // We rely on onFocus for history saving or explicit undo points.
  ...
  // Generic content change wrapper
  const updateContent = (field: keyof SiteContent, value: string) => {
    setContent(prev => ({ ...prev, [field]: value }));
  };

  return (
    <div className="fixed inset-0 z-50 bg-obsidian-100 flex flex-col font-sans" dir="rtl">
      {/* Top Bar */}
      <div className="h-16 bg-white border-b border-obsidian-200 flex items-center justify-between px-6 shadow-sm z-50">
        <div className="flex items-center gap-4">
          <h2 className="text-xl font-bold font-serif text-obsidian-900">CMS Panel</h2>
          <div className="h-6 w-px bg-obsidian-200"></div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <Undo2 size={18} />
            <span className="text-xs font-bold hidden md:inline">בטל</span>
          </button>
          ...
          >
            <XCircle size={18} />
            <span className="text-xs font-bold hidden md:inline">בטל שינויים</span>
          </button>

          <div className="h-6 w-px bg-obsidian-200 mx-2"></div>

          <button onClick={handleReset} className="p-2 text-obsidian-400 hover:text-red-500 hover:bg-red-50 rounded-full" title="איפוס מפעל (מחיקת הכל)">
            <RotateCcw size={18} />
          </button>
          
          <button onClick={handleSave} className="flex items-center gap-2 px-4 py-2 bg-gold-500 text-white rounded-md hover:bg-gold-600 font-bold shadow-sm mx-2">
            <Save size={18} /> שמירה
          </button>
          
          <button onClick={onClose} className="p-2 hover:bg-obsidian-100 rounded-full text-obsidian-500">
            <X size={24} />
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* Left Sidebar - Editor */}
        <div className="w-[400px] bg-white border-l border-obsidian-200 overflow-y-auto p-6 shadow-xl z-40">
          
          {activeTab === 'projects' ? (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                 <label className="text-sm font-bold text-obsidian-500 uppercase tracking-wider">בחר פרויקט לעריכה</label>
                 <button onClick={handleAddProject} className="text-xs bg-gold-50 text-gold-600 px-2 py-1 rounded border border-gold-200 hover:bg-gold-100 flex items-center gap-1">
                    <Plus size={12}/> חדש
                 </button>
              </div>
              ...
              {selectedProject ? (
                <div className="space-y-4 animate-fade-in">
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-obsidian-400">כותרת</label>
                    <input 
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-obsidian-400">קטגוריה</label>
                        <input 
                    ...
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-obsidian-400">אייקון (Lucide)</label>
                        <input 
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-obsidian-400">קישור לאתר (URL)</label>
                    <input 
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-obsidian-400">תיאור</label>
                    <textarea 
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-obsidian-400">מדיה (URL תמונה/וידאו בשורות נפרדות)</label>
                    <textarea 
                      ...
                      dir="ltr"
                    />
                    <p className="text-[10px] text-gray-400">תומך ב-JPG, PNG, MP4, YouTube</p>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <label htmlFor="autoplay" className="text-sm">ניגון גלריה אוטומטי</label>
                  </div>

                  <div className="space-y-1">
                    <label className="text-xs font-bold text-obsidian-400">טכנולוגיות (מופרד בפסיק)</label>
                    <input 
                  ...
                  <div className="pt-4 border-t border-obsidian-100">
                    <button onClick={handleDeleteProject} className="text-red-500 text-sm flex items-center gap-1 hover:underline">
                        <Trash size={14} /> מחק פרויקט זה
                    </button>
                  </div>
                </div>
              ) : (
                <div className="text-center text-gray-400 py-10">בחר פרויקט לעריכה</div>
              )}
            </div>
          ) : (
            /* Global Content Tab */
            <div className="space-y-6 animate-fade-in">
                <h3 className="font-bold text-lg text-obsidian-800">הגדרות כותרת (Hero)</h3>
                
                <div className="space-y-1">
                    <label className="text-xs font-bold text-obsidian-400">כותרת עליונה</label>
                    <input 
                ...
                <div className="space-y-1">
                    <label className="text-xs font-bold text-obsidian-400">כותרת משנה (אפקט)</label>
                    <input 
                ...
                <div className="space-y-1">
                    <label className="text-xs font-bold text-obsidian-400">טקסט אודות</label>
                    <textarea 
                ...
                <div className="h-px bg-obsidian-100 my-4"></div>
                <h3 className="font-bold text-lg text-obsidian-800">ציטוט ופוטר</h3>

                <div className="space-y-1">
                    <label className="text-xs font-bold text-obsidian-400">משפט השראה (ציטוט)</label>
                    <textarea 
                ...
                <div className="space-y-1">
                    <label className="text-xs font-bold text-obsidian-400">מחבר הציטוט</label>
                    <input 
                ...
                <div className="space-y-1">
                    <label className="text-xs font-bold text-obsidian-400">כותרת פוטר</label>
                    <input 
                ...
                <div className="space-y-1">
                    <label className="text-xs font-bold text-obsidian-400">טקסט פוטר</label>
                    <textarea 
        ...
        {/* Right Side - Preview */}
        <div className="flex-1 bg-obsidian-100 flex flex-col min-w-0">
          {/* Toolbar */}
          <div className="h-10 bg-white border-b border-obsidian-200 flex justify-center items-center gap-4">
            <button 
          ...
          <div className="flex-1 overflow-auto p-8 flex justify-center bg-gray-200/50">
            ...
            >
               {/* Simulate iframe-like isolation for scroll */}
